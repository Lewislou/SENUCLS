...
####
def _post_proc_para_wrapper(pred_map_mmap_path, inst,tile_info, func, func_kwargs):
    """Wrapper for parallel post processing."""
    idx, tile_tl, tile_br = tile_info
    wsi_pred_map_ptr = np.load(pred_map_mmap_path, mmap_mode="r")
    #print(wsi_pred_map_ptr.shape,idx, tile_tl, tile_br)
    tile_pred_map = wsi_pred_map_ptr[tile_tl[0] : tile_br[0], tile_tl[1] : tile_br[1]]
    tile_inst_map = inst[tile_tl[0] : tile_br[0], tile_tl[1] : tile_br[1]]
    tile_pred_map = np.array(tile_pred_map)  # from mmap to ram
    #print('tile_pred_map max: ',tile_pred_map.max())
    tile_inst_map = np.array(tile_inst_map)  # from mmap to ram
    return func(tile_pred_map,tile_inst_map, **func_kwargs), tile_info
...
def _assemble_and_flush(wsi_pred_map_mmap_path, chunk_info, patch_output_list):
    ...
    for pinfo in patch_output_list:
        ...
        #print('pdata: ',pdata.shape,pdata.max())
        chunk_pred_map[
...
class InferManager(base.InferManager):
    def __run_model(self, patch_top_left_list, pbar_desc):
        ...
        for batch_idx, batch_data in enumerate(dataloader):
            ...
            sample_inst_list = []
            batch_data ={}
            for (i,j) in sample_info_list:
                inst = self.hover_pred_inst_map[i:i+sample_data_list[0].shape[0],j:j+sample_data_list[0].shape[1]].astype("int32")
                relab, forward_map, inverse_map = segmentation.relabel_sequential(inst)
                sample_inst_list.append(relab)
            batch_data['img'] = sample_data_list
            batch_data["inst_map"] = torch.tensor(sample_inst_list)
            sample_output_list = self.run_step(batch_data)
            #print(sample_output_list.shape)
            #print(sample_output_list.max())
            #sample_output_list = sample_output_list.reshape(1,sample_output_list.shape[0],sample_output_list.shape[1])
            sample_info_list = sample_info_list.numpy()
            curr_batch_size = sample_output_list.shape[0]
            #print(sample_output_list.shape,sample_info_list.shape,curr_batch_size)
    ...
    def __dispatch_post_processing(self, tile_info_list, callback):
        ...
        print('Post processing initialisation')
        proc_pool = None
        ...
        future_list = []
        wsi_pred_map_mmap_path = "%s/pred_map.npy" % self.cache_path
        #sprint('tile_info_list',tile_info_list.shape)
        for idx in list(range(tile_info_list.shape[0])):
            ...
            if proc_pool is not None:
                ...
                results = _post_proc_para_wrapper(
                    wsi_pred_map_mmap_path,self.hover_pred_inst_map, tile_info, self.post_proc_func, func_kwargs
                )
                callback(results)
        if proc_pool is not None:
            silent_crash = False
    ...
    def process_single_file(self, wsi_path, inst_pred_path,msk_path, output_dir):
        ...
        self.hover_pred_inst_map = np.load(inst_pred_path +wsi_name+ '.npy')
        ...
        print("Preparing Input Output Placement: {0}".format(end - start))
    ...
    def process_wsi_list(self, run_args):
        ...
        #print(self.input_dir,wsi_path_list)
        for wsi_path in wsi_path_list[:]:
            ...
            try:
                ...
                self.process_single_file(wsi_path,self.inst_pred_dir, msk_path, self.output_dir)
                log_info("Finish")
            except:
